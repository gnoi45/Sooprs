...
$db = $con; // enter your connection variable
...
function pagination_records($totalRecordsPerPage, $tableName)
{
  ...
  if (isset($_GET['search_professional'])) {
    $search_key = $_GET['search_professional'];
    // print_r($search_key);
    // exit();
    $query = $db->prepare("SELECT * FROM " . $tableName . " WHERE name LIKE ". $search_key. " LIMIT ?, ?");
    // print_r($result);
    // exit();
  }else{
...
function pagination($totalRecordsPerPage, $tableName)
{
  ...
  if (isset($_GET['search_professional'])) {
    $search_key = $_GET['search_professional'];
    $query = "SELECT * FROM " . $tableName." WHERE name LIKE ".$search_key;
  }else{
