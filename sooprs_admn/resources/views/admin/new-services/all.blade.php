            ...
            <div class="row">
                ...
                <div class="col-sm-12 col-lg-12">
                    ...
                    <div class="mb-3 mt-3 ">
                        ...
                        <a href="{{ route('add.new.service') }}" type="button" class="btn  btn-info text-white" >Add New</a>
                    ...
                    <table id="example" class="table table-striped" style="width:100%">
                        ...
                        <tbody>
                            @foreach ($allNewServices as $single)
                                <tr>

                                    {{-- <td>{{ $single->id }}</td> --}}
                                    <td >{{ $single->service_name }} </td>
                                    <td >{{ $single->service_slug }} </td>
                                    <td >
                                        @if($single->status == 0)
                                    ...
                                    <td >
                                        @if($single->service_bg_img !== null)
                                        <img src="{{ $single->service_bg_img }}" alt="service-image" style="width:70px">
                                        @else

                                        @endif
                                    </td>
                                    <td >{!! $single->service_desc !!} </td>
                                   <td>{{ $single->parentCategory }}</td>
                                    ...
                                    @can('service-edit')
                                        <td>
                                            <div class="d-flex">
                                                <a href="{{ route('edit.new.service', ['id'=>$single->id]) }}" type="button" title="Edit"  class="btn btn-sm btn-success  text-white me-2"  >
                                                    Edit
                                                    {{-- <i class="mdi mdi-delete"></i>            --}}
                                                </a>
                                                <a href="{{ route('remove.service', $single->id) }}" type="button" title="delete"  class="btn btn-sm btn-danger text-white  "  onclick="return confirm('Are you sure you want to delete this ?')">
                                                    Delete
                                                    {{-- <i class="mdi mdi-delete"></i>            --}}
                                                </a>
                                            </div>
