...
function gist_custom_class($classes)
{
    ...
    $sidebar = !empty($gist_theme_options['gist-sidebar-options']) ? $gist_theme_options['gist-sidebar-options'] : '';
...
function gist_breadcrumb_options(){
    global $gist_theme_options;
    if( 1 == $gist_theme_options['gist-extra-breadcrumb'] && !is_front_page()){
        gist_breadcrumbs();
    }
}
endif;
...
if( ! function_exists( 'gist_breadcrumbs' ) ):
    function gist_breadcrumbs() {
       ...
       global $gist_theme_options;
        ...
        $breadcrumb_from = esc_html( $gist_theme_options['gist-breadcrumb-display-from-option'] );
        $breadcrumb_plugin = esc_html( $gist_theme_options['gist-breadcrumb-display-from-plugins'] );

        if($breadcrumb_from == 'theme-default') {
