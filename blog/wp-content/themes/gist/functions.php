...
function gist_scripts() {
	...
	$gist_name_font_url   = !empty($gist_theme_options['gist-font-url']) ? esc_url( $gist_theme_options['gist-font-url'] ) : '';
	...
	if( !empty($gist_theme_options['gist-sticky-sidebar'] )  && 1 == $gist_theme_options['gist-sticky-sidebar'] ){
 ...
 */
function gist_block_styles() {

	wp_enqueue_style( 'gist-editor-styles', get_theme_file_uri( 'candidthemes/assets/css/editor-styles.css' ) );

	/*heading  */
    wp_enqueue_style('gist-editor-heading-font', '//fonts.googleapis.com/css?family=Merriweather:30');

    /*heading  */
    wp_enqueue_style('gist-editor-heading-font', '//fonts.googleapis.com/css?family=Oswald');
